...
exports.login = async (req, res, next) => {
	...
	if (!user) {
		...
		if (!user) {
			return res.status(401).json({ error: 'Authentication invalid, please try again', isLogin: true });
		}
...
exports.refresh = async (req, res, next) => {
	...
	jwt.verify(accessToken, process.env.JWT_SECRET, { ignoreExpiration: true }, async function(err, payload) {
		const { id } = payload;
		let user = await Customer.findById(id);
		if (!user) {
			user = await Banker.findById(id);
			if (!user) res.status(400).json({ success: false, err: 'User not exists' });
		}
